...
export const useNovel = (novelOrPath: string | NovelInfo, pluginId: string) => {
  ...
  const getChapters = useCallback(async () => {
    ...
    console.log('[useNovel.getChapters] Starting fetch:', { novelId: novel?.id, page, novelPath, pluginId });
    ...
    if (novel && page) {
      ...
      console.log('[useNovel.getChapters] Chapter count from DB:', chapterCount);

      if (chapterCount) {
        try {
          console.log('[useNovel.getChapters] Loading chapters from database cache');
          newChapters = getPageChaptersBatched(...config) || [];
          console.log('[useNovel.getChapters] Loaded from cache:', newChapters.length);
        } catch (error) {
          console.error('[useNovel.getChapters] Error loading from cache:', error);
        }
      }
      // Fetch next page if no chapters
      else if (Number(page)) {
        console.log('[useNovel.getChapters] No chapters in DB, fetching from plugin');
        _setChapters([]);
        try {
          console.log('[useNovel.getChapters] Calling fetchPage:', { pluginId, novelPath, page });
          const sourcePage = await fetchPage(pluginId, novelPath, page);
          console.log('[useNovel.getChapters] Received chapters from plugin:', sourcePage.chapters?.length);
          ...
          console.log('[useNovel.getChapters] Inserting chapters to database:', sourceChapters.length);
          await insertChapters(novel.id, sourceChapters);
          console.log('[useNovel.getChapters] Chapters inserted, loading from DB');
          newChapters = await _getPageChapters(...config);
          chapterCount = getChapterCount(novel.id, page);
          console.log('[useNovel.getChapters] Loaded from DB after insert:', newChapters.length);
        } catch (error) {
          console.error('[useNovel.getChapters] Error fetching chapters from plugin:', error);
          throw error;
        }
      }
      ...
      console.log('[useNovel.getChapters] Setting chapters:', newChapters.length);
      setChapters(newChapters);
    } else {
      console.log('[useNovel.getChapters] Skipping - missing novel or page', { hasNovel: !!novel, hasPage: !!page });
    }
  }, [
    novel,
    novelPath,
  ...
  useEffect(() => {
    ...
    console.log('[useNovel.useEffect] Starting to fetch chapters for novel:', { novelId: novel.id, novelPath });
    setFetching(true);
    getChapters()
      .catch(e => {
